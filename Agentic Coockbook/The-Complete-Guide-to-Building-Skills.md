# The Complete Guide to Building AI Skills
...
A skill is a set of instructions — packaged as a simple folder — that teaches an AI model how to handle specific tasks or workflows. Skills are one of the most powerful ways to customize AI for your specific needs. Instead of re-explaining your preferences, processes, and domain expertise in every conversation, skills let you encode that knowledge once and benefit every time.

Skills are powerful when you have repeatable workflows: generating frontend designs from specs, conducting research with consistent methodology, creating documents that follow your team's style guide, or orchestrating multi-step processes. They work well alongside the model's built-in capabilities like code execution and document creation. For those building MCP integrations, skills add another powerful layer — helping turn raw tool access into reliable, optimized workflows.
...
- Developers who want an AI model to follow specific workflows consistently
- Power users who want consistent, repeatable AI behavior
- Teams looking to standardize how AI works across their organization
...
**What you'll get out of this guide:** By the end, you'll be able to build a functional skill in a single sitting. Expect about 15–30 minutes to build and test your first working skill.
...
- **First level (YAML frontmatter):** Always loaded in the model's system prompt. Provides just enough information for the model to know when each skill should be used without loading all of it into context.
- **Second level (SKILL.md body):** Loaded when the model determines the skill is relevant to the current task. Contains the full instructions and guidance.
- **Third level (Linked files):** Additional files bundled within the skill directory that the model can navigate and discover only as needed.

This progressive disclosure minimizes token usage while maintaining specialized expertise.

#### Composability

Multiple skills can be loaded simultaneously. Your skill should work well alongside others, not assume it's the only capability available.
...
If you already have a working MCP server, you've done the hard part. Skills are the knowledge layer on top — capturing the workflows and best practices you already know, so the model can apply them consistently.
...
|---|---|
| Connects the AI to your service (Notion, Asana, Linear, etc.) | Teaches the model how to use your service effectively |
| Provides real-time data access and tool invocation | Captures workflows and best practices |
| **What** the model can do | **How** the model should do it |
...
- Users blame your connector when the real issue is missing workflow guidance
...
- No external tools required — uses the model's built-in capabilities
...
> "Interactive guide for planning development sprints. Walks the user through use case definition, task breakdown, capacity estimation, and validation."
...
> These are aspirational targets — rough benchmarks rather than precise thresholds. Aim for rigor but accept that there will be an element of vibes-based assessment. More robust measurement guidance and tooling continues to evolve across the ecosystem.
...
- **Users don't need to prompt the model about next steps**
...
- When distributing via a public repo, add a repo-level README for human visitors — this is separate from the skill folder itself.

---

### YAML Frontmatter: The Most Important Part

The YAML frontmatter is how the model decides whether to load your skill. Get this right.
...
- XML angle brackets (`< >`) — may be used to inject instructions
- Reserved or trademarked names per your platform's guidelines

> **Why:** Frontmatter is injected into the model's system prompt. Malicious content in frontmatter could inject instructions or manipulate model behavior.
...
The description provides just enough information for the model to know when each skill should be used without loading all of it into context. This is the first level of progressive disclosure.
...
1. Verify MCP server is running (check your platform's extension/connector settings)
2. Confirm API key is valid
3. Try reconnecting your service integration
```
...
- **Manual testing** — Run queries directly in an interactive session and observe behavior. Fast iteration, no setup required.
- **Scripted testing** — Automate test cases for repeatable validation across changes.
...
>
> The most effective skill creators iterate on a single challenging task until the model succeeds, then extract the winning approach into a skill. This leverages the model's in-context learning and provides faster signal than broad testing. Once you have a working foundation, expand to multiple test cases for coverage.
...
### Bootstrapping Skills with AI Assistance

Many AI platforms include tools or skills for building further skills. If yours does, you can describe your top 2–3 workflows and generate a first draft in a single sitting — often in 15–30 minutes.
...
- After encountering edge cases or failures, bring those examples back and ask for targeted improvements
- Example: *"Use the issues identified in this conversation to improve how the skill handles [specific edge case]"*

> **Note:** AI-assisted bootstrapping helps you design and refine skills but does not execute automated test suites or produce quantitative evaluation results.
...
3. Install via your AI platform's skills or capabilities settings
4. Or place in the appropriate skills directory for CLI/agentic tools

**Organization-level skills:**
- Admins can deploy skills workspace-wide via platform admin controls
- Automatic updates
- Centralized management

### An Open Standard

Agent Skills is an open standard. Like MCP, the goal is for skills to be portable across tools and platforms — the same skill should work regardless of which AI platform is used. That said, some skills are designed to take full advantage of a specific platform's capabilities; authors can note this in the skill's `compatibility` field.
...
**Key capabilities:**
- API endpoints for listing and managing skills
- Attach skills to inference requests
- Version control and management through your platform's console
- Works with agent SDKs for building custom agents

**When to use skills interactively vs. via API:**

| Use Case | Best Surface |
|---|---|
| End users interacting with skills directly | Interactive platform (chat UI) |
| Manual testing and iteration during development | Interactive platform (chat UI) |
| Individual, ad-hoc workflows | Interactive platform (chat UI) |
| Applications using skills programmatically | API |
| Production deployments at scale | API |
| Automated pipelines and agent systems | API |

> **Note:** Skills running via API typically require a sandboxed execution environment. Check your platform's documentation for requirements.
...
2. Install in your AI platform:
   - Open the platform's Settings > Skills (or Capabilities)
   - Upload the skill folder (zipped)
...
4. Test:
   - Ask the AI: "Set up a new project in [Your Service]"
```
...
> "Our MCP server gives the AI access to your Linear projects. Our skills teach it your team's sprint planning workflow. Together, they enable AI-powered project management."
...
- **Tool-first:** "I have Notion MCP connected" → Your skill teaches the model the optimal workflows and best practices. Users have access; the skill provides expertise.
...
**Debugging approach:** Ask the AI: "When would you use the [skill name] skill?" It will quote the description back. Adjust based on what's missing.
...
1. **Verify MCP server is connected**
   - Check your platform's extension/connector settings
...
3. **Test MCP independently**
   - Ask the AI to call the MCP tool directly (without the skill)
   - "Use [Service] MCP to fetch my projects"
   - If this fails, the issue is with MCP, not the skill
...
If you're building your first skill, start with your platform's best practices guide and skill examples, then reference the API docs as needed.

### Official Documentation

Look for the following from your AI platform of choice:
- Skills / Instructions / Custom behavior documentation
- API reference for attaching skills to requests
- MCP documentation
...
- Many platforms include a built-in skill for creating further skills
- Can generate a skill from a natural language description
- Reviews descriptions and flags common issues
- Ask: "Help me build a skill for [your use case]"

**Validation:**
- Ask the AI to review your draft skill and suggest improvements
- Ask: "Review this skill and identify any issues with triggering or instructions"

### Patterns and Examples

Look for public skill repositories from your platform provider or the community. Good examples to look for:
- Document creation skills (PDF, DOCX, PPTX)
- Workflow automation skills
- Partner/integration skills (e.g., for Asana, Figma, Sentry, Zapier)

Clone, modify, and use them as starting templates.

### Getting Support

- Check your platform's developer community forums or Discord
- File issues against the relevant skill repository with: skill name, error message, and steps to reproduce

---

## Reference A: Quick Checklist

Use this checklist to validate your skill before and after installation. If you want a faster start, use an AI-assisted skill generator to produce a first draft, then run through this list to make sure you haven't missed anything.
...
**Forbidden:**
- XML angle brackets (`< >`) — may be used to inject instructions into the system prompt
- Code execution in YAML (uses safe YAML parsing)
- Reserved or trademarked names as defined by your platform (e.g., platform brand names)

---

## Reference C: Complete Skill Examples

For full, production-ready skills demonstrating the patterns in this guide, look for public repositories from your platform provider. Typical categories include:

- **Document Skills** — PDF, DOCX, PPTX, XLSX creation
- **Example Skills** — Various workflow patterns
- **Partner Skills** — Skills from integrations such as Asana, Atlassian, Canva, Figma, Sentry, Zapier, and more

Clone them, modify them for your use case, and use them as starting templates.
